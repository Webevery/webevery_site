import defaultCoworker from '../images/ourTeamCompressed/defaultCoworker.webp';
import volodymyrKavun from '../images/ourTeamCompressed/volodymyrKavun.webp';
import maksPeltek from '../images/ourTeamCompressed/maksPeltek.webp';
import nataliiaBoiko from '../images/ourTeamCompressed/nataliiaBoiko.webp';
import ivikaDuda from '../images/ourTeamCompressed/ivikaDuda.webp';
import andriiTsapkov from '../images/ourTeamCompressed/andriiTsapkov.webp';
import dmytroKolodchenko from '../images/ourTeamCompressed/dmytroKolodchenko.webp';
import maksPadalka from '../images/ourTeamCompressed/maksPadalka.webp';
import vitaliiPavlovskyi from '../images/ourTeamCompressed/vitaliiPavlovskyi.webp';
import myroslavKozar from '../images/ourTeamCompressed/myroslavKozar.webp';
// import volodymurPolztyuk from '../images/ourTeam/volodymurPolztyuk.png';
// import maksymIgnatenko from '../images/ourTeam/maksymIgnatenko.png';
import oksanaRomanenko from '../images/ourTeamCompressed/oksanaRomanenko.webp';
import ivanGagaus from '../images/ourTeamCompressed/ivanGagaus.webp';
import denysKudriavets from '../images/ourTeamCompressed/denysKudriavets.webp';
import dmitroKotenev from '../images/ourTeamCompressed/dmitroKotenev.webp';

import defaultCoworkerSmall from '../images/ourTeamCompressed/defaultCoworkerSmall.webp';
import volodymyrKavunSmall from '../images/ourTeamCompressed/volodymyrKavunSmall.webp';
import maksPeltekSmall from '../images/ourTeamCompressed/maksPeltekSmall.webp';
import nataliiaBoikoSmall from '../images/ourTeamCompressed/nataliiaBoikoSmall.webp';
import ivikaDudaSmall from '../images/ourTeamCompressed/ivikaDudaSmall.webp';
import andriiTsapkovSmall from '../images/ourTeamCompressed/andriiTsapkovSmall.webp';
import dmytroKolodchenkoSmall from '../images/ourTeamCompressed/dmytroKolodchenkoSmall.webp';
import maksPadalkaSmall from '../images/ourTeamCompressed/maksPadalkaSmall.webp';
import vitaliiPavlovskyiSmall from '../images/ourTeamCompressed/vitaliiPavlovskyiSmall.webp';
import myroslavKozarSmall from '../images/ourTeamCompressed/myroslavKozarSmall.webp';
// import volodymurPolztyukSmall from '../images/ourTeam/volodymurPolztyukSmall.png';
// import maksymIgnatenkoSmall from '../images/ourTeam/maksymIgnatenkoSmall.png';
import oksanaRomanenkoSmall from '../images/ourTeamCompressed/oksanaRomanenkoSmall.webp';
import ivanGagausSmall from '../images/ourTeamCompressed/ivanGagausSmall.webp';
import denysKudriavetsSmall from '../images/ourTeamCompressed/denysKudriavetsSmall.webp';
import dmitroKotenevSmall from '../images/ourTeamCompressed/dmitroKotenevSmall.webp';

import mapGdansk from 'images/locationsMaps/mapGdansk.png';
import mapIvanoFrankivsk from 'images/locationsMaps/mapIvano-Frankivsk.png';
import mapKamianetsPodilskyi from 'images/locationsMaps/mapKamianets-Podilskyi.png';
import mapKharkiv from 'images/locationsMaps/mapKharkiv.png';
import mapKyiv from 'images/locationsMaps/mapKyiv.png';
import mapLandauinderPfalz from 'images/locationsMaps/mapLandau-in-der-Pfalz.png';
import mapLviv from 'images/locationsMaps/mapLviv.png';
import mapOdesa from 'images/locationsMaps/mapOdesa.png';
import mapOstrava from 'images/locationsMaps/mapOstrava.png';
import mapSumy from 'images/locationsMaps/mapSumy.png';
// import mapTallinn from 'images/locationsMaps/mapTallinn.png';
import mapTrenchin from 'images/locationsMaps/mapTrenchin.png';
import mapUjgorod from 'images/locationsMaps/mapUjgorod.png';
import mapZaporizhzhia from 'images/locationsMaps/mapZaporizhzhia.png';

export const coworkersDataEN = [
  {
    id: '0',
    name: 'Nataliia Boiko',
    photo: nataliiaBoiko,
    photoSmall: nataliiaBoikoSmall,
    profession: 'Full Stack Developer',
    quote:
      'It’s worth remembering that small steps, rather than giant leaps, often lead to the most lasting changes.',
    humor: 'You can take this button and put it there, and it will be good!',
    city: 'Ivano-Frankivsk',
    latitude: 48.92312,
    longitude: 24.71248,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Ivano-Frankivsk',
    map: mapIvanoFrankivsk,
    email: 'natalabojko@gmail.com',
    phone: +380503738465,
  },

  {
    id: '1',
    name: 'Volodymyr Kavun',
    photo: volodymyrKavun,
    photoSmall: volodymyrKavunSmall,
    profession: 'Full Stack Developer',
    quote:
      'When you have a dream, you have to grab it and never let go, and do your best to make it come true every day.',
    humor: 'I feel that I need to tweak and adjust it a little more.',
    city: 'Ostrava',
    // cityEn: 'Ostrava',
    latitude: 49.83465,
    longitude: 18.28204,
    country: 'Czech Republic',
    // countryEn: 'Czech Republic',
    location: 'Ostrava',
    map: mapOstrava,
    email: 'volodymirkavun1@gmail.com',
    phone: +380636876176,
  },

  {
    id: '2',
    name: 'Maksim Peltek',
    photo: maksPeltek,
    photoSmall: maksPeltekSmall,
    profession: 'Full Stack Developer',
    quote:
      'Do not compare yourself to anyone. Hold your head high and remember: You are not better, not worse, you are you, and no one can deny it.',
    humor: 'I have doubts again. Can we do this? Can I do THIS?',
    city: 'Odesa',
    // cityEn: 'Odesa',
    latitude: 46.381806,
    longitude: 30.71039,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Odesa',
    map: mapOdesa,
    email: 'peltek1985@gmail.com',
    phone: +380683396082,
  },

  {
    id: '3',
    name: 'Ivika Duda',
    photo: ivikaDuda,
    photoSmall: ivikaDudaSmall,
    profession: 'Full Stack Developer',
    quote:
      'The bad news is that time flies. The good news: You are the pilot, and everything depends on you.',
    // quoteEn: 'The bad news is time flies. The good news is you are the pilot.',
    humor: 'The further we go, the worse it gets. It’s time to change direction.',
    city: 'Ujgorod',
    // cityEn: 'Ujgorod',
    latitude: 48.6242,
    longitude: 22.2947,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Ujgorod',
    map: mapUjgorod,
    email: 'kivikaivika@gmail.com',
    phone: +380955834998,
  },

  {
    id: '4',
    name: 'Andrii Tsapkov',
    photo: andriiTsapkov,
    photoSmall: andriiTsapkovSmall,
    profession: 'Full Stack Developer',
    quote: 'There are many things in life that a reasonable person would not want to pay attention to.',
    // quoteEn: 'There is nothing impossible to they who will try.',
    humor: 'People are willing to do anything for money, even to work.',
    city: "Kamianets-Podilskyi",
    // cityEn: 'Kamianets-Podilskyi',
    latitude: 48.67882,
    longitude: 26.58516,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Kamianets-Podilskyi',
    map: mapKamianetsPodilskyi,
    email: 'a.tsapkov.dev@gmail.com',
    phone: +380936452142,
  },

  {
    id: '5',
    name: 'Dmytro Kolodchenko',
    photo: dmytroKolodchenko,
    photoSmall: dmytroKolodchenkoSmall,
    profession: 'Full Stack Developer',
    quote: 'Keep your face always toward the sunshine, and shadows will fall behind you.',
    // quoteEn:
    //   'Keep your face always toward the sunshine, and shadows will fall behind you.',
    humor: 'Everyone is endowed with a brain, but not everyone understands the instructions.',
    city: 'Zaporizhzhia',
    // cityEn: 'Zaporizhzhia',
    latitude: 47.85167,
    longitude: 35.11714,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Zaporizhzhia',
    map: mapZaporizhzhia,
    email: 'dima.kolod@gmail.com',
    phone: +380953907792,
  },

  {
    id: '6',
    name: 'Maksym Padalka',
    photo: maksPadalka,
    photoSmall: maksPadalkaSmall,
    profession: 'Full Stack Developer',
    quote: 'Success is not final, failure is not fatal. What matters is the courage to continue.',
    // quoteEn:
    // 'Success is not final, failure is not fatal: it is the courage to continue that counts.',
    humor: 'I earn a lot, but get little.',
    city: 'Trenchin',
    // cityEn: 'Trenchin',
    latitude: 48.89452,
    longitude: 18.04436,
    country: 'Slovakia',
    // countryEn: 'Slovakia',
    location: 'Trenchin',
    map: mapTrenchin,
    email: 'sheihmaks@gmail.com',
    phone: +380671915300,
  },

  {
    id: '7',
    name: 'Vitalii Pavlovskyi',
    photo: vitaliiPavlovskyi,
    photoSmall: vitaliiPavlovskyiSmall,
    profession: 'Full Stack Developer',
    quote: 'Remember, you are the master of your life. Never let other people write your script.',
    humor: 'There are many good people, but few useful ones.',
    city: 'Sumy',
    // cityEn: 'Sumy',
    latitude: 50.9077,
    longitude: 34.7981,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Sumy',
    map: mapSumy,
    email: 'sledozar@gmail.com',
    phone: +380664710207,
  },

  {
    id: '8',
    name: 'Volodymyr Polztiuk',
    photo: defaultCoworker,
    photoSmall: defaultCoworkerSmall,
    profession: 'Full Stack Developer',
    quote: 'You are never too old to set another goal or dream a new dream. It all depends on you.',
    humor: 'I don’t know how, but I’ll figure it out!',
    city: 'Lviv',
    // cityEn: 'Lviv',
    latitude: 49.83826,
    longitude: 24.02324,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Lviv',
    map: mapLviv,
    email: 'wowa.pol@ukr.net',
    // phone: +380000000000,
  },

  {
    id: '9',
    name: 'Myroslav Kozar',
    photo: myroslavKozar,
    photoSmall: myroslavKozarSmall,
    profession: 'Full Stack Developer',
    quote: 'Theory during training is very good, but there should be eighty percent practice.',
    humor: 'Ahhhhh, I GET IT...',
    city: 'Landau-in-der-Pfalz',
    // cityEn: 'Landau-in-der-Pfalz',
    latitude: 49.19844,
    longitude: 8.11692,
    country: 'Germany',
    // countryEn: 'Germany',
    location: 'Landau-in-der-Pfalz',
    map: mapLandauinderPfalz,
    email: 'myrkozar@gmail.com',
    phone: +380500172671,
  },

  {
    id: '10',
    name: 'Oksana Romanenko',
    photo: oksanaRomanenko,
    photoSmall: oksanaRomanenkoSmall,
    profession: 'UX/UI Designer',
    quote: 'When a flower does not bloom, you are not fixing the flower, but the environment in which it grows.',
    humor: 'Where it’s YELLOW, but not the one that’s YELLOW.',
    city: 'Gdansk',
    // cityEn: 'Gdansk',
    latitude: 54.372158,
    longitude: 18.638306,
    country: 'Poland',
    // countryEn: 'Poland',
    location: 'Gdansk',
    map: mapGdansk,
    email: 'oks.romanenkodesign@gmail.com',
    phone: +380676861493,
  },

  {
    id: '11',
    name: 'Ivan Gagaus',
    photo: ivanGagaus,
    photoSmall: ivanGagausSmall,
    profession: 'UX/UI Designer',
    quote: 'Every day I get up and look at the Forbes list of the world’s richest people. If I’m not there, I go to work.',
    humor: 'The number of minds on the planet is a constant, the population is growing.',
    city: 'Odesa',
    // cityEn: 'Odesa',
    latitude: 46.48572,
    longitude: 30.74383,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Odesa',
    map: mapOdesa,
    // email: '',
    // phone: +3801111111,
  },

  {
    id: '12',
    name: 'Denys Kudriavets',
    photo: denysKudriavets,
    photoSmall: denysKudriavetsSmall,
    profession: 'QA Engineer',
    quote: 'You don’t always need a plan.Sometimes you just need to breathe, trust, let go and see what happens.',
    humor: 'I am selling a nervous system, in excellent condition, starts from a half turn.',
    city: 'Kharkiv',
    // cityEn: 'Kharkiv',
    latitude: 50.00156,
    longitude: 36.231537,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Kharkiv',
    map: mapKharkiv,
    email: 'kudriavetsdenis@gmail.com',
    phone: +380686345365,
  },

  {
    id: '13',
    name: 'Dmitro Kotenov',
    photo: dmitroKotenev,
    photoSmall: dmitroKotenevSmall,
    profession: 'UX/UI Designer',
    quote: 'Human life is a collection of different moments. So collect them like the best photographs.',
    humor: 'Go against the wind. And let them spit in your back.',
    city: 'Kyiv',
    // cityEn: 'Kyiv',
    latitude: 50.450001,
    longitude: 30.523333,
    country: 'Ukraine',
    // countryEn: 'Ukraine',
    location: 'Kyiv',
    map: mapKyiv,
    email: 'kotenevdima1@gmail.com',
    phone: +380632468706,
  },
  // {
  //   id: '14',
  //   name: 'Maksym Ignatenko',
  //   // photo: maksymIgnatenko,
  //   // photoSmall: maksymIgnatenkoSmall,
  //   profession: 'Full Stack Developer',
  //   quote: 'Wake up determined, go to bed satisfied.',
  //   humor: '',
  //   city: 'Tallinn',
  // //   cityEn: 'Tallinn',
  //   latitude: 59.436962,
  //   longitude: 24.753574,
  //   country: 'Естонія',
  // //   countryEn: 'Estonia',
  //   location: 'Tallinn',
  //   map: mapTallinn,
  //   email: 'cadillac.maxim@gmail.com',
  //   phone: +380664770959,
  // },
];
