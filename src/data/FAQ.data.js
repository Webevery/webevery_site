import styles from '../components/screens/HowWeWorkPage/FAQ/FAQItem/FAQItem.module.scss';

export const FAQdata = [
  {
    id: 1,
    question: 'Скільки коштує розробка програмного продукту?',
    answer:
      'Наш досвід дозволяє проконсультувати та розрахувати бюджет і терміни саме під Ваші потреби, враховуючи всі можливі ризики. Для цього зв’яжіться з нами та поділіться своїми ідеями. Більш детальну інформацію ви можете дізнатись у розділі ',
    link: '"Послуги".',
    href: '/services',
    answerHeight: styles.first,
  },
  {
    id: 2,
    question: 'Скільки часу знадобиться, щоб отримати готовий продукт?',
    answer:
<<<<<<< HEAD
      'Необхідні часові рамки повністю залежать від вашого проекту. Першим кроком після звернення до нас є організація безкоштовної онлайн-зустрічі, яка дозволить нам зрозуміти складність вашого проекту. Коли ми зрозуміємо ваші вимоги до програмного забезпечення, ми зможемо надати попередню інформацію про витрати. Проте початкові терміни не завжди дотримуються, оскільки зовнішні обставини можуть змінити випуск кінцевого продукту. Потім ми надамо план реалізації вашого проекту та кроки, які ми пропонуємо виконати, перш ніж фактично почати роботу над вашим програмним рішенням.',
    answerHeight: styles.second,
=======
      'Необхідні часові рамки повністю залежать від Вашого проєкту. Першим кроком після звернення до нас є організація безкоштовної онлайн-зустрічі, яка дозволить нам зрозуміти складність Вашого проєкту. Коли ми зрозуміємо Ваші вимоги до програмного забезпечення, ми зможемо надати попередню інформацію про витрати. Проте, початкові терміни не завжди дотримуються, оскільки зовнішні обставини можуть змінити випуск кінцевого продукту. Потім ми надаємо план реалізації Вашого проекту та кроки, які ми пропонуємо виконати, перш ніж фактично почати роботу над Вашим програмним рішенням.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 3,
    question:
      'Чи потрібно мені бути технічно підкованим, щоб працювати з вами?',
    answer:
<<<<<<< HEAD
      'Звісно ж ні. Ми тут, щоб допомогти вам технічно реалізувати будь-які потреби вашого бізнесу, навіть без вашого технічного досвіду. Наявність технічного спеціаліста на вашому боці вітається, але не обов’язково. У нас можна простою мовою отримати поради щодо складних речей. Ми надаємо повну підтримку від першої зустрічі до кінцевого продукту і можемо навіть підтримувати його після.',
    answerHeight: styles.third,
=======
      'Звісно ж ні. Ми тут, щоб допомогти Вам технічно реалізувати будь-які потреби Вашого бізнесу, навіть без Вашого технічного досвіду. Наявність технічного спеціаліста на Вашому боці це добре, але не обов’язково. У нас можна простою мовою отримати поради щодо складних речей. Ми надаємо повну підтримку від першої зустрічі до кінцевого продукту і можемо навіть підтримувати його після завершення.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 4,
    question: 'Як Ви гарантуєте якість продукції?',
    answer:
<<<<<<< HEAD
      'Ми ділимо весь етап розробки на спринти, які зазвичай тривають два тижні. Після кожного спринту ви можете побачити готові частини продукту, перевірити їх і висловити свої думки та пропозиції. У нас також є спрощений процес тестування та забезпечення якості кожного продукту. Ви отримаєте проміжні результати і побачите, за що платите протягом усього періоду розробки.',
    answerHeight: styles.fourth,
=======
      'Ми ділимо весь етап розробки на спринти, які, зазвичай, тривають два тижні. Після кожного спринту ви можете побачити готові частини продукту, перевірити їх і висловити свої думки та пропозиції. У нас також є спрощений процес тестування та забезпечення якості кожного продукту. Ви отримаєте проміжні результати і побачите, за що платите протягом усього періоду розробки.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 5,
    question: 'Що, якщо я хочу брати повну участь у процесі розробки?',
    answer:
<<<<<<< HEAD
      'Це можливо і навіть потрібно. Разом ми створимо продукт, який, безсумнівно, вирішить ваші проблеми та допоможе бізнесу. У нас прозорі процеси, ви знаєте кожного члена команди, який працює над тим, щоб зробити ваш продукт реальним. Ви також можете брати участь у наших плануваннях, ретроспективах і отримувати проміжні звіти про етапи розробки, перевіряти їх і пропонувати свої ідеї.',
    answerHeight: styles.fifth,
=======
      'Це можливо і навіть потрібно. Разом ми створимо продукт, який, безсумнівно, вирішить Ваші проблеми та допоможе бізнесу. У нас прозорі процеси, Ви знаєте кожного члена команди, який працює над тим, щоб зробити Ваш продукт реальним. Ви також можете брати участь у наших плануваннях, ретроспективах і отримувати проміжні звіти про етапи розробки, перевіряти їх і пропонувати свої ідеї.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 6,
    question:
      'Чи можна оновити програмне забезпечення в майбутньому, коли з’являться нові технології?',
    answer:
<<<<<<< HEAD
      'Звичайно! Крім того, наші фахівці мають гарний досвід у таких справах. Ми проконсультуємо вас щодо низки технологій і необхідних рішень, щоб ваш продукт відповідав вимогам ринку та залишався сучасним.',
    answerHeight: styles.sixth,
=======
      'Звичайно! Крім того, наші фахівці мають гарний досвід у таких справах. Ми проконсультуємо Вас щодо низки технологій і необхідних рішень, щоб Ваш продукт відповідав вимогам ринку та залишався сучасним.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 7,
    question: 'Що, якщо я прийду до вас із продуктом, який хочу вдосконалити?',
    answer:
<<<<<<< HEAD
      'Якщо у вас вже є продукт і ви хочете його вдосконалити, розкажіть нам про ваші основні цілі та причини вашого звернення. Ми обговоримо ваші потреби, вивчимо ваш продукт, проведемо аудит і складемо план вдосконалення всього продукту або його окремих компонентів.',
    answerHeight: styles.seventh,
=======
      'Якщо у Вас вже є продукт і Ви хочете його вдосконалити, розкажіть нам про Ваші основні цілі та причини Вашого звернення. Ми обговоримо Ваші потреби, вивчимо Ваш продукт, проведемо аудит і складемо план вдосконалення всього продукту або його окремих компонентів.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 8,
    question: 'Що станеться після того, як Ви закінчите мій продукт?',
    answer:
<<<<<<< HEAD
      'Продовження роботи над проектом залежить від ваших подальших цілей. Ми завжди пропонуємо послуги підтримки та обслуговування продукту, якщо клієнт зацікавлений. Коли продукт стане масштабним, ми можемо організувати для вас спеціальну команду, яка розробить продукт і забезпечить його високу якість.',
    answerHeight: styles.eighth,
=======
      'Продовження роботи над проєктом залежить від Ваших подальших цілей. Ми завжди пропонуємо послуги підтримки та обслуговування продукту, якщо клієнт зацікавлений. Коли продукт стане масштабним, ми зможемо організувати для Вас спеціальну команду, яка розробить продукт і забезпечить його високу якість.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 9,
    question:
      'Чи гарантуєте Ви безпеку та конфіденційність нашої інтелектуальної власності?',
    answer:
<<<<<<< HEAD
      'Так, саме тому наші клієнти довіряють нам. Ми прописуємо всі пункти щодо безпеки конфіденційності та інтелектуальної власності в Основних угодах про надання послуг і нерозголошення між нашою і вашою компанією. Конфіденційність, приватність і захист вашої інтелектуальної власності є для нас пріоритетом номер один.',
    answerHeight: styles.ninth,
=======
      'Так. Саме тому наші клієнти довіряють нам. Ми прописуємо всі пункти щодо безпеки конфіденційності та інтелектуальної власності в Основних угодах про надання послуг і нерозголошення між нашою і Вашою компанією. Конфіденційність, приватність і захист Вашої інтелектуальної власності є для нас пріоритетом номер один.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 10,
    question: 'Як Ви обробляєте запити клієнтів на зміни?',
    answer:
      'Ми маємо стандартну процедуру обробки запитів на зміни від наших клієнтів. Перш за все, ми просимо їх схвалити будь-які запити на зміни. Після того, як клієнт схвалив запит, ми проводимо аналіз впливу. Це дозволяє нам надати замовнику терміни та вартість змін. Після затвердження вартості замовником ми вносимо зміни.',
    answerHeight: styles.tenth,
  },
  {
    id: 11,
    question: 'Чи надаєте Ви послуги тестування програмного забезпечення?',
    answer:
<<<<<<< HEAD
      'Звичайно! У нас налагоджені всі процеси тестування. Кожен продукт проходить ретельну перевірку як під час розробки, так і на завершальному етапі.',
    answerHeight: styles.eleventh,
=======
      'Звичайно! У нас налагоджені всі процеси тестування. Кожен продукт проходить ретельну перевірку, як під час розробки, так і на завершальному етапі.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
  {
    id: 12,
    question:
      'Ви працюєте тільки віддалено чи можете приїхати до мене за потреби?',
    answer:
      'Зазвичай ми працюємо віддалено і маємо для цього налаштовані всі необхідні процеси. Також можливий виїзд наших спеціалістів до Вас в офіс. Місцезнаходження наших спеціалістів Ви можете подивитись у розділі ',
    link: '"Команда".',
    href: '/ourTeam',
    answerHeight: styles.twelfth,
  },
  {
    id: 13,
    question: 'Яка процедура початку процесу розробки?',
    answer:
<<<<<<< HEAD
      'Перш за все, ми вивчаємо ваші вимоги і пропонуємо приблизну вартість і терміни. Отримавши підтвердження з вашого боку, ми підписуємо договір, формуємо команду, яка буде працювати над вашим проектом і починаємо процес розробки.',
    answerHeight: styles.thirteenth,
=======
      'Перш за все, ми вивчаємо Ваші вимоги і пропонуємо приблизну вартість і терміни. Отримавши підтвердження з Вашого боку, ми підписуємо договір, формуємо команду, яка буде працювати над вашим проєктом, і починаємо процес розробки.',
>>>>>>> 6fba70a0e0d6d8a7d3066e07de13bf73e3dd0d2b
  },
];
